...
describe('Post API:', function () {
  ...
  describe('POST /api/posts', function () {
    beforeEach(function (done) {
      (0, _supertest2.default)(app).post('/api/posts').send({
        name: 'New Post',
  ...
  describe('PUT /api/posts/:id', function () {
    ...
    beforeEach(function (done) {
      (0, _supertest2.default)(app).put('/api/posts/' + newPost._id).send({
        name: 'Updated Post',
  ...
  describe('DELETE /api/posts/:id', function () {
    ...
    it('should respond with 204 on successful removal', function (done) {
      (0, _supertest2.default)(app).delete('/api/posts/' + newPost._id).expect(204).end(function (err, res) {
        if (err) {
          return done(err);
        }
        done();
    ...
    it('should respond with 404 when post does not exist', function (done) {
      (0, _supertest2.default)(app).delete('/api/posts/' + newPost._id).expect(404).end(function (err, res) {
        if (err) {
          return done(err);
        }
        done();
